...
func  (rf *Raft)AE (i int) bool {
	...
	if nexti< len(rf.log)&&rf.lastapplied[i]!=-1{
		...
		if rf.lastapplied[i]<10&& len(rf.log)>200{
			m=200
		}else {
			m= len(rf.log)-1
		}
...
func (rf *Raft)Worker()  {
	...
	for {
		...
		if rf.ident==leader {
			...
			term11:=rf.term
			go rf.Vote()
			for  !rf.killed()&&rf.ident==candidate&&rf.term==term11{
				rf.mu.Unlock()
				select {// 需要for
				case <-time.After((time.Duration(rand.Int63()%15*40+300))*time.Millisecond):
					rf.mu.Lock()
					if rf.ident==candidate&&rf.term==term11&&!rf.killed(){
						rf.term++
						rf.persist()
					}
			...
			for !rf.killed()&&rf.ident==follower&&rf.term==term3 {
				rf.mu.Unlock()
				select {
				case <-time.After((time.Duration(rand.Int63()%15*80+150))*time.Millisecond):
					rf.mu.Lock()
