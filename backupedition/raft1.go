	...
	for i:=0;i<n;i++{
		...
		go func(i int) {
			...
			if Voterpl.Termcurrent>rf.term  {
				...
				rf.bcfollower<-bcfl1
				return
			}else if Voterpl.Requestterm!=rf.term{
				rf.mu.Unlock()
				return
			}
			rf.mu.Unlock()
			...
			if req.Prevlogt<rf.log[req.Prevlogi].Term{
				for j:=req.Prevlogi-1;j>0;j--{
					if rf.log[j].Term<=req.Prevlogt {
						reply.Nextindex=j+1
						fmt.Println("comm1",reply.Nextindex)
					}
				}
				reply.Nextindex=1
				...
				if len(rf.log)>req.Prevlogi+1&&req.Lostsofent!=nil{
	...
	for {
		...
		if rf.ident==leader {
			...
			if ok&&rf.term==term11{
				...
				rf.mu.Unlock()
				select {
				case <-time.After((time.Duration(rand.Int63()%20*40+150))*time.Millisecond):
					rf.mu.Lock()
