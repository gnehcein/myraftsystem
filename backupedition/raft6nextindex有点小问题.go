...
func (rf *Raft)Vote() {
	...
	for {
		...
		case <-vote:
			...
			if votenum>=n/2{
				select {
				case <-time.After(10*time.Millisecond):		//因为不知道worker的状态，可能不是candidate了，就没有bcleader通道
...
func (rf *Raft)AppendEntries(req *AppendEntries,reply *Appendreply){
	...
	rf.mu.Lock()
	//fmt.Println("arglen",len(req.Lostsofent))
	//fmt.Println("loglen",len(rf.log))
		...
		if len(rf.log)>req.Prevlogi {
			...
			if req.Prevlogt<rf.log[req.Prevlogi].Term{
				...
				if req.Lostsofent==nil{
					...
					if len(rf.log)-1-req.Prevlogi> len(req.Lostsofent){
						...
						for index:=0;index< len(req.Lostsofent);index++{
							...
							if req.Lostsofent[index].Term!=rf.log[req.Prevlogi+1+index].Term {
								rf.log=rf.log[:req.Prevlogi+1+index]
								for t:=index;t< len(req.Lostsofent);t++{
						...
						rf.log=rf.log[:req.Prevlogi+1]	//直接截断
						for t:=0;t< len(req.Lostsofent);t++{
							rf.log=append(rf.log,req.Lostsofent[t])
						}
						rf.persist()
					}
					reply.Nextindex=req.Prevlogi+ len(req.Lostsofent)+1
...
func  (rf *Raft)AE (i int) {
	...
	if nexti< len(rf.log)&&rf.lastapplied[i]!=-1{	//否则传的lostsofent为nil
		var m  int		//m是lostsofent最后的log 对应在rf.log的entry的index
		//如果是已经和follower达成之前的（index小的）日志一致，那么就一次全传过去;否则寻找一致的时候，只传一个entry
		m= int(math.Min(float64(nexti+100), float64(len(rf.log)-1))) //最多传100个
		for k:=nexti;k<=m;k++{
	...
	if rep.CurTerm>rf.term{
		rf.mu.Unlock()
		rf.bcfollower<-bcfl{rep.CurTerm}
		return
	}
	if rep.ReqTerm==rf.term&&rep.CurTerm==rf.term {		//用newnext进行对两种情况的整合
		if rep.Baseterm>0 {
...
func (rf *Raft)Worker()  {
	...
	for {
		...
		if rf.ident==leader {
			...
			rf.mu.Unlock()
			for i:=0;i<n ;i++ {
				fmt.Println(rf.nextindex[i],"nextindex")
				fmt.Println(rf.lastapplied[i],"lastapplied")
				if i==me{
					continue
				}
				go rf.AE(i)
			...
			for  !rf.killed()&&rf.ident==candidate&&rf.term==term2{
				rf.mu.Unlock()
				select {
				case <-time.After((time.Duration(rand.Int63()%20*50+300))*time.Millisecond):
					rf.mu.Lock()
			...
			for !rf.killed()&&rf.ident==follower&&rf.term==term3 {
				rf.mu.Unlock()
				select {
				case <-time.After((time.Duration(rand.Int63()%20*40+300))*time.Millisecond):
					rf.mu.Lock()
